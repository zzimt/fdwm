...
Display* getglobaldisplay(void) { return dpy; }

void setglobaldisplay(Display* display) { dpy = display; }
...
void monocle(Monitor* m) {
    unsigned int n = 0;
    Client* c;
    char oldltsymbol[sizeof m->ltsymbol] = { 0 };
    memcpy(oldltsymbol, m->ltsymbol, sizeof m->ltsymbol);

    for (c = m->clients; c; c = c->next)
        if (ISVISIBLE(c))
            n++;
    if (n > 0) { /* override layout symbol */
        snprintf(m->ltsymbol, sizeof m->ltsymbol, "%s(%d)", oldltsymbol, n);
    }
    ...
    do {
        ...
        case MotionNotify:
            ...
            if ((unsigned int)abs(selmon->wx - nx) < snap)
                nx = selmon->wx;
            else if ((unsigned int)abs(
                         (selmon->wx + selmon->ww) - (nx + WIDTH(c))) < snap)
                nx = selmon->wx + selmon->ww - WIDTH(c);
            if ((unsigned int)abs(selmon->wy - ny) < snap)
                ny = selmon->wy;
            else if ((unsigned int)abs(
                         (selmon->wy + selmon->wh) - (ny + HEIGHT(c))) < snap)
                ny = selmon->wy + selmon->wh - HEIGHT(c);
            if (!c->isfloating && selmon->lt[selmon->sellt]->arrange &&
                ((unsigned int)abs(nx - c->x) > snap ||
                    (unsigned int)abs(ny - c->y) > snap))
                togglefloating(NULL);
    ...
    do {
        ...
        case MotionNotify:
            ...
            if (c->mon->wx + nw >= selmon->wx &&
                ...
                if (!c->isfloating && selmon->lt[selmon->sellt]->arrange &&
                    ((unsigned int)abs(nw - c->w) > snap ||
                        (unsigned int)abs(nh - c->h) > snap))
                    togglefloating(NULL);
            }
            if (!selmon->lt[selmon->sellt]->arrange || c->isfloating)
...
void setup(void) {
    ...
    XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
        PropModeReplace, (unsigned char*)"fdwm", 3);
...
void spawn(const Arg* arg) {
    ...
    if (fork() == 0) {
        ...
        die("fdwm: execvp '%s' failed:", ((char**)arg->v)[0]);
    }
}
...
int xerror(Display* dpy, XErrorEvent* ee) {
    if (ee->error_code == BadWindow ||
        ...
        return 0;
    fprintf(stderr, "fdwm: fatal error: request code=%d, error code=%d\n",
...
int xerrorstart(Display* dpy, XErrorEvent* ee) {
    ...
    die("fdwm: another window manager is already running");
    return -1;
}
